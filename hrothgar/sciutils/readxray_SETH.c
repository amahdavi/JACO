...
{
  fitsfile *fin, *fbkg, *phafile;
  int status=0, chancol, errcol;
  int i, j, chansize;
  double meanflux, meanerr;
  long nrows;
  int no_stat_err=0, type, repeat, width;
  double arcminarea=0., exposure=0.;
  double bkg_arcminarea=0., bkg_exposure=0, back_k=0.;
  char comment[100];
  ...
  fits_read_key_dbl(phafile,"BACKSCAL",&arcminarea,comment,&status);
  if (status) BYE("Cannot read BACKSCAL keyword");
  fits_read_key_dbl(phafile,"EXPOSURE",&exposure,comment,&status);
  if (status) BYE("Cannot read EXPOSURE keyword");
...
/*   result->stat_err = sci_dvector(result->nchannels); */
  ...
  /* Change the COUNTS column from long to double in order to hold the 
   		non-integer counts predicted by the model */
  fits_get_coltype(phafile, chancol, &type, &repeat, &width, &status);
  if (type != TDOUBLE)
  {
	fits_delete_col(phafile, chancol, &status);
	fits_insert_col(phafile, chancol, "COUNTS", "1D", &status);
	fits_get_coltype(phafile, chancol, &type, &repeat, &width, &status);
  }
  if (status) BYE("cfitsio errors %d",status);
  ...
  if (status)	
  {
	status=0;
	// no_stat_err=1;
	/* Add STAT_ERR column if it does not exist.  This will result
			in simulations with errors equal to the errors on the data.
			If you would like to instead compute the errors from the 
			model, then comment out the lines below and uncomment 
			the line above. */
	fits_insert_col(phafile, chancol+1, "STAT_ERR", "1D", &status);
	fits_get_colnum(phafile,CASEINSEN,"STAT_ERR",&errcol,&status);
	if (status)
	{
		no_stat_err=1;
  ...
  /* Will add back in the background counts */
  struct pha *background = (struct pha*) result->background;
  if (result->backname)
  {	
	fits_open_file(&fbkg,result->backname,READONLY,&status);
	if (status) BYE("cfitsio errors %d",status);
	fits_movnam_hdu(fbkg,BINARY_TBL,"SPECTRUM",0,&status);
	if (status) BYE("cfitsio errors %d",status);
	fits_read_key_dbl(fbkg,"BACKSCAL",&bkg_arcminarea,comment,&status);
	if (status) BYE("Cannot read BACKSCAL keyword");
	fits_read_key_dbl(fbkg,"EXPOSURE",&bkg_exposure,comment,&status);
	if (status) BYE("Cannot read EXPOSURE keyword");
	fits_close_file(fbkg,&status);
	if (status) BYE("cfitsio errors %d",status);
	
	back_k = exposure*arcminarea/(bkg_exposure*bkg_arcminarea);
  }
  /* Siegel End */
  ...
  for (i = 0; i < result->nbins; ++i) 
  {
	...
	meanflux = result->exposure*result->mcounts[i];
	if (result->backname)
	{
		for (j = result->chanlo[i]; j <= result->chanhi[i]; ++j)	
			meanflux += back_k*background->counts[j];
	}
	meanflux /= chansize;
    meanerr = result->exposure*sqrt(result->sig[i]*result->sig[i]/chansize);
    
    for (j = result->chanlo[i]; j <= result->chanhi[i]; ++j)
	{
      fits_write_col_dbl(phafile,chancol,j+1,1,1,&meanflux,&status);
      if (!no_stat_err) fits_write_col_dbl(phafile,errcol,j+1,1,1,&meanerr,&status);
    }
  }
  if (status) BYE("cfitsio errors %d",status);
	     ...
	     double efit1, double efit2, double syserr)  
{
  ...
  for (i = chanstart; i <= chanstop; ++i) 
    if (spec->quality[i] == 0) {
      ...
      if (spec->backname) {
		...
		if (!spec->is_sim)
			spec->stat_err[i] = sqrt(spec->counts[i]+back_k*back_k*background->counts[i]);
      }
  ...
  for (i = 0; i < spec->nbins; ++i) {
    ...
    spec->sigsq[i] /= expsq;
    if (!spec->is_sim) spec->sigsq[i] += pow(spec->flux[i]*syserr,2.0);
...
float **readimage_subset(char *fname, float *pixscale,
		  ...
		  long xcen, long ycen, float dist)
{
  fitsfile *fptr;
...
float **readimage(char *fname, float *pixscale,
		  unsigned long *nx, unsigned long *ny)
{
  return readimage_subset(fname, pixscale, nx, ny, -1, -1, 0.);
}
