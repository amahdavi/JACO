...
void write_minimize_output(gsl_vector *p,struct hrothgar_setup *setup)
{
  ...
  for (i = 0; i < setup->ntotparams; ++i) {
    ...
    if (!p) {
      fprintf(setup->outfile,"%-12s %12.10E %5d   ",
      ...
      if (setup->frozen[i])
	fprintf(setup->outfile,"%12.10E ",setup->params[i]);
      else
	fprintf(setup->outfile,"%12.10E ",
...
void lord_minimize(struct hrothgar_setup *setup)
{
  ...
  minfunc.df = NULL;//&minimize_func_der;
  minfunc.fdf = NULL;//&minimize_func_funcder;
  ...
  if (!setup->covaronly) {
	...
	for (i = 0; i < nparsfloat; ++i) {
	  ...
	  j = setup->far[par];
	  tparams[par] +=
	    gsl_ran_gaussian(setup->generator,setup->gaussfrac);
...
int hrothgar_init(struct hrothgar_setup *setup,int argc, char *argv[])
{
  ...
  for (i = 0; i < conflines; ++i) {
    ...
    if (parname[i]) {
      ...
      // First try to determine if it is a string parameter.
	...
	if (fp == 0) {
	  ...
	  if ((isnan(tmppmin[i]) && !pmin) || (isnan(tmppmax[i]) && !pmax))
	    BYE("%s: Neither %s nor the calling code specifies parameter limits.",
		pn, argv[optind]);

	  if (frz[i] < 0) {
	    hrothgar_printf("Warning: %s fit state unspecified---assuming frozen\n",pn);
	    frz[i] = 1;
	  }
  ...
  if (setup->covaronly) {
    ...
    for (i = 0; i < setup->ntotparams; ++i) {
      float del = (setup->pmax[i]-setup->pmin[i]);
      setup->mcmcstep[i] = 
	setup->gaussfrac*(fabs(setup->params[i]) >  0.01*del ?
			  fabs(setup->params[i]) :
			  del);
    }
...
/*     } */
    ...
    if (!finite(chisq)) {
      BYE("Encountered non-finite chisq: %E\n",chisq);
    }
