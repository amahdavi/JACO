...
typedef struct BigSZControlParams BigSZControlParams ;
#endif
...
#define MAXANN   100        // Maximum number of annuli
...
struct jaco_state {
  ...
  double lxe1, lxe2;           // Lower and upper rest-frame energy in which to report Lx
  ...
  // Nonthermal pressure support (fractional)
  double nonthflag;           // flag to include nonthermal pressure support
  double nonthoutnorm;        // A: Pnonthermal_outer/Ptotal = 1 - A*(1 + exp(-(r/(B*r200m))^C)
  double nonthoutscale;       // B: Pnonthermal_outer/Ptotal = 1 - A*(1 + exp(-(r/(B*r200m))^C)
  double nonthoutslope;       // C: Pnonthermal_outer/Ptotal = 1 - A*(1 + exp(-(r/(B*r200m))^C)
  double nonthinnorm;         // D: Pnonthermal_inner/Ptotal = D*(1 + (r/(E*r200m))^4)^(-F/4)
  double nonthinscale;        // E: Pnonthermal_inner/Ptotal = D*(1 + (r/(E*r200m))^4)^(-F/4)
  double nonthinslope;        // F: Pnonthermal_inner/Ptotal = D*(1 + (r/(E*r200m))^4)^(-F/4)
  ...
  int fitfgas;                   // Whether to interpret Mg1
                               // as total mass (=0) or total 
                               // gas fraction;
  ...
  double rhoref;               // Reference density for overdensity radius routine (Siegel)
  ...
  double rhomatter;            // Mean matter density of universe at z (Siegel)
  ...
  char *wlcov;                 // Weak lensing covariance file (Siegel)
  ...
  double *szrel_corr_pressure; // Pressure corrected for relativistic effects		// Siegel
  ...
  char *szcov;                 // fits file containing SZ covariance matrix
  double szy[100000];          // Tabulated Compton y
  double szrad[100000];        // Radius at which it's tabulated
  int lastszannulus;
  ...
  gsl_interp *pinterp_coefs;	 // Interpolators for pressure			// Siegel
  gsl_interp *szinterp_coefs;    // Interpolators for relativistic corrected pressure
...
struct jaco_vars {
  ...
  gsl_interp_accel *paccel;	  // Pressure interpolation accelerator			// Siegel
  gsl_interp_accel *szaccel;  // SZ pressure interpolation accelerator
...
/* Siegel Begin */
#define GAS_NAGAI             4  // Gas density model from Nagai
#define GAS_MORANDI           3  // Gas density model from Morandi et al.(2012)
/* Siegel End */
...
void  jaco_sz(int annulus, struct jaco_vars *jv);
