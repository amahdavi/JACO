...
// The following global is necessitated by the Sherpa calling conventions
struct jaco_state sherpa_jaco_state;
...
{
  int  nwllist, nwldata, nszconf, ii;
  char **wllist,**wldata,**szconf,*endptr,*ext;
  ...
  js->simimage = stringparval[SIMIMAGE];
  ...
  /* Siegel Begin */
  nwldata = parse_list(stringparval[WLDATA],',',&wldata);

  if (nwldata < 0 || nwldata > 2)
    BYE("Error parsing weak lensing parameters %s",stringparval[WLDATA]);
  ...
  if (nwldata == 2)
  {	
	ext = strrchr(wldata[1], '.');
	if (ext == NULL)
		BYE("Error parsing weak lensing covariance %s",stringparval[WLDATA]);
  ...
  /* Siegel Begin */
  nszconf = parse_list(stringparval[SZCONFIGS],',',&szconf);

  if (nszconf < 0 || nszconf > 2)
    BYE("Error parsing szconf parameters %s",stringparval[SZCONFIGS]);

  js->szconfig = szconf[0];
  js->szcov = NULL;

  if (nszconf == 2)
  {	
	ext = strrchr(szconf[1], '.');
	if (ext == NULL)
		BYE("Error parsing sz covariance %s",stringparval[SZCONFIGS]);
  ...
  if (js->szcov) printf("SZ covariance matrix:  %s\n", js->szcov);
  /* Siegel End */

  js->xrayra=atof(stringparval[XRAYRA]);
  js->xraydec=atof(stringparval[XRAYDEC]);
...
/* <<<<<<< sherpa.c */
/*     count = read_ascii(js->jacofilename,"%s %f %f",NULL, */
		     ...
		     double *x0, struct jaco_state *js)
{
  ...
  int i,j,k,l,annulus,samepars=1,sameeverything=1,coresame=0,newmodel,debug;
  int recalcback = 0,id,instr;
  ...
  if ((int)params[ANNULUS] == 0)
    if (firstinstr < 0) firstinstr = (int)params[INSTRUMENT];
  ...
  for (i = 1; i < NTOTPARAMS; ++i) {
    ...
    // Do the checking only if neither parameter is close to zero
    js->changedpar[i] = (fabs(params[i]-oldp[i]) > 0.);
    if (i < ANNULUS) samepars *= 1-js->changedpar[i];
  ...
  // Normally, the BETAs are restricted to be greater than some positive
  // value, e.g. 0.4. If the model is a broken power law, however, the
  // first BETA could well go all the way to zero.
  if (abs((int)params[GASMODEL]) == GAS_BROKENPOWERLAW) parmin[BETA1] = 0.;

  /* Siegel Begin */
  // Similarly, if we are using the Morandi model for the gas density, 
  // then BETA3 can go all the way to zero.
  //if (abs((int)params[GASMODEL]) == GAS_MORANDI) parmin[BETA3] = 0.;
  /* Siegel End */
  
  // Do not allow exploration beyond bounds specified in the init
  // code.  This includes nonfinite values!  This is particularly
  // helpful when we are dealing with Sherpa's projection routines.
  for (i = 1; i < NTOTPARAMS; ++i) {
    if (params[i] < parmin[i] || 
	params[i] > parmax[i] ||
	!FINITE(params[i])) {
      if (debug > 2) 
	printf("disallowing limit breach: %d %E (%E to %E)\n",
	       i,params[i],parmin[i],parmax[i]);
      if (params[i] > parmax[i]) params[i] = parmax[i];
      if (params[i] < parmin[i]) params[i] = parmin[i];
    }
  }
  ...
  samepars = 0;
  // The following kludgy lines of code are required because of a
  // technical limitation of Sherpa. The first model ever evaluated
  // needs to unnecessarily evaluated multiple times throughout the
  // analysis. This is to make sherpa include models with linked
  // parameters within the fit. To avoid taking a performance hit
  // because of this, the model corresponding to the very first
  // spectrum is saved in its own memory space.
  if (!js->standalone) {
    if ((int)params[ANNULUS] == 0 && firstinstr == (int)params[INSTRUMENT]) {
      if (savespectrum == NULL) savespectrum = sci_dvector(*ndata);
      if (sameeverything) {
	for (i = 0; i < *ndata; ++i)
	  outValues[i] = savespectrum[i];
	return 0;
      }
    }
  }
  ...
  annulus=(int)params[ANNULUS]; 
  instr=(int)params[INSTRUMENT];

  newmodel = !annulus;
  if (newmodel) donexray = 0;
  
  // Running the system commands below can help diagnose memory leaks
  // quickly:
  // system("echo sherpa.c in : `ps -eo fname,rss | grep sherpa`");
  ...
  if (instr == SZ) {
    if (js->szparams.nest != *ndata) {
      jaco_printf("ERROR -- mismatch in length of SZ data. %d vs. %d\n",
	     js->szparams.nest,*ndata);
      return -1;
    }
    if (js->dofast) {
      if (savesz == NULL || newmodel || !donexray && js->dofast) {
	jaco_printf("ERROR --- in fast mode you need an X-ray model before the SZ model.\n");
	return -1;
      } else 
...
#pragma omp parallel private(i,j) firstprivate(jv) shared(js,spectrum,ebins,instr) default(none)
  ...
  for (j = 0; j < *ndata; ++j) {  
    ...
    if (annulus == 0 && instr == firstinstr && !js->standalone) 
      savespectrum[j] = finalspectrum[j];
  ...
  //if (donepars)
  //  system("echo ----sherpa.c out: `ps -eo fname,rss | grep sherpa`");
  if (debug > 0)
    fclose(js->logfile);
...
int ascfitusermodelinit_()
{
  static int doneinit=0;
  int argc = 3;
  struct hrothgar_setup setup;

  char *argv[] = {"jaco","-E",DEFAULTCONFIGFILE};

  if (doneinit) return 0;

  jaco_init(&sherpa_jaco_state,&setup,argc,argv);
  sherpa_jaco_state.standalone=0;
  doneinit=1;

  return jaco_read_data(&sherpa_jaco_state);
}

/* Parameter initialization routine. */
/* Returns 0 for success, 1 for failure. */
/* Input argument: i, the index of the parameter */
/* Output arguments: */
/* value: initial value of parameter */
/* min: initial lower limit of parameter value (when fitting) */
/* max: initial upper limit of parameter value (when fitting) */
/* name: name of parameter (max 32 chars) */
/* descr: description of parameter (max 32 chars) */
/* units: units of parameter (max 32 chars) */
/* state: state of parameter (0 means Frozen, 1 means Thawed) */
/* The string lengths nameLen, descLen, and unitsLen are used for the
fortran version of the interface, and can be ignored in the  c interface */

/* This routine gets called once for each parameter (except the */
/* parameter count, which is parameter 0) right after the allocation */
/* of the parmeters in saofit. When you set the parameter count */
/* in saofit the parameters get allocated, and this routine is */
/* called. */
/* ====================================================================== */
int ascfitusermodelsetparamvalues_(int* i, 
				   ...
				   double*  max, 
				   char* name, 
				   char* descr __attribute__ ((unused)), 
				   char* units, 
				   int* state, 
				   int* nameLen __attribute__ ((unused)), 
				   int* descLen __attribute__ ((unused)), 
				   int* unitsLen __attribute__ ((unused)))
{

  if (*i > 0) {
    *min = parmin[*i]; 
    *max = parmax[*i];
    *state = (*i == MG1 || *i == DARKMASS || *i == RX1 || *i == RX2 ||
	      *i == RDM1 || *i == Z0);
    *value = initvalues[*i];
    strcpy(name,paramnames[*i]);
    strcpy(units,paramunits[*i]);
  }
...
/* Returns 0 for success, 1 for failure. */
/* Input argument:  */
/* parameterList, the 0th entry is the parameter count */
/* data: array of data bins defining the dataspace of the image */
/* numberOfData: size of data */
/* Output arguments: */
/* outValues: array of size numberOfData, which should be filled by the routine */

/* This function gets called once for every iteration of the optimization */
/* method through parameter space. This could be MANY times, so be  */
/* careful to make this routine as efficient as possible, and not to */
/* allocate any memory if you can avoid it. If you do allocate, remember */
/* to delete it before the return of the function. */
/* ====================================================================== */
// NB -- For pha spectra, sherpa formats x0 such that each item marks the
// START of a series of REGULARLY SPACED bins
int ascfitusermodelfunction_(double* params,
			     int* numberOfDimensions __attribute__ ((unused)),
			     int* ndata,
			     double* outValues,
			     double* x0,
			     double* x1 __attribute__ ((unused)),
			     double* x2 __attribute__ ((unused)),
			     double* x3 __attribute__ ((unused)),
			     double* x4 __attribute__ ((unused)),
			     double* x5 __attribute__ ((unused)),
			     double* x6 __attribute__ ((unused)))
{

  return jaco_calc_models(params,ndata,outValues,x0,&sherpa_jaco_state);
}

/* Cleanup routine. */

/* This routine gets called once if/when the userModel is deleted  */
/* in saofit. Any memory allocated in the init() routine should be */
/* deleted here. Open files should be closed, \etc */
/* ====================================================================== */
void ascfitusermodelcleanup_()
{
  /* here you should get rid of any storage space allocated in init
   look in the static example to see how */
  deallocatevectors();
}
