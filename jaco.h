...
#define BYE(format...) { printf("JACO Error:\n"); printf(format); printf("\n"); exit(-1); }
...
struct jaco_state {
  ...
  int gasmodel;                // GAS_SPHERICAL or GAS_TRIAXIAL; 
                               // Sign of gas model indicates whether Md measures total or dark
  ...
  float pixscale;              // Pixel scale of simulated image arcsec/pix


  // Triaxial parameters
  double asq,bsq;             // Square of axis ratios.
  double x_x,x_y,x_z,
    y_x,y_y,y_z,
    z_x,z_y,z_z;              // Components of Euler rotation matrix
  char *ximage;               // X-ray images name
  double xime1,xime2;         // Lower/upper bound of X-ray image
  float **softimage;          // Soft X-ray image
  double **softmodel;         // Soft X-ray image
  float **softexp;            // Soft X-ray exposure mask
  double ***xymodel;          // Main spatially resolved 2D X-ray model
  unsigned long xnx, xny;     // 2D XY model dimensions
  float xpixscale;            // X-ray Pixel scale in arcsec/pixel
  float wcs[3][2];            // X-ray WCS information
  double cosdec0,sindec0,
    cosra0,sinra0;
  double ximnorm,ximback;     // Soft X-ray image parameters
  long int nsoftdata;
...
struct jaco_vars {
  ...
  double x,y;                 // For triaxial mode, the x and y values in Mpc
  ...
  double *integrand_coefs;    // For triaxial integration--coefficients
  double *integrand_ordinate; // For triaxial integration--y values
  int dolog;                  // Whether interpolated quantities are log-interpolated.
...
#define TRIAXIAL              1  // Triaxial model
#define SPHERICAL             2  // Spherical model

#define SOFTIMAGE_MIN         1 // Minimum photon counts to consider
                                 // in X-ray image mode
...
int jaco_calc_models(int instrument,
		     int annulus,
